...
@dataclass
class ExtractSeriesArgs:
    """A Dataclass to wrap extract_series arguments"""
...
class DataTransformer:
    ...
    def extract_series(self, args: ExtractSeriesArgs) -> pd.DataFrame:
        """Extract a series from source_data"""

        args = args.__dict__

        series_columns = args["series_columns"]
        series_name = args["series_name"]
        series_keyword = args["series_keyword"]
        source_data = args["source_data"]
        immutable_columns = args["immutable_columns"]
        multiple_index = args["multiple_index"]
        ...
        # Change the date to 31/12
        data.loc[:, "Year"] = (
            data["Year"] - pd.DateOffset(days=1) + pd.DateOffset(years=1)
        )
